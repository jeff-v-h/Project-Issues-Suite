...
export function isBrowserSupported() {
  if ((typeof(Storage) === 'undefined') || CRYPTOBJ === undefined || CRYPTOBJ.getRandomValues === 'undefined' ) {
    return false;
  } else {
    return true;
  }
...
export function createStringConstants(...constants) {
  return constants.reduce((acc, constant) => {
    ...
    if (typeof constant !== 'string')throw new Error(
...
export function getTicketsForDropdown(tickets) {
  return tickets.map(ticket =>
    ({ text: ticket.name, value: ticket.name })
  );
 }
...
function isNameAllowed(name) {
  return isOnlyWhitespace(name) || hasUnsafeCharacters(name) ||
