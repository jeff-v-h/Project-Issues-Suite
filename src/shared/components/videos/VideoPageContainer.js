...
import { getTicketByName, updateTicket, selectVideo } from '../../actions/tickets-actions';
import PageHeader from '../common/PageHeader';
import EditVideoForm from './EditVideoForm';
import DeleteModal from '../common/DeleteModal';
import NoteInput from './NoteInput';
import NoteList from './NoteList';
import { nameCheck, getPath, encodeUrlComponent, decodeUrlComponent } from '../../../helpers/utils';
...
class VideoPageContainer extends React.PureComponent {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (prevProps.tickets.selected != tickets.selected) {
      const video = tickets.selected.videos.find(v => v.title == this.state.vidTitle);
  ...
  handleSubmit = e => {
    ...
    const titleChanged = video.title != ticket.video.title;
    const titleExists = ticket.videos.some(
      v => v.title.toUpperCase() == video.title.toUpperCase());
    if (titleChanged && titleExists) {
      this.setState({ titleError: true });
      toastr.error(`Title '${video.title}' already exists for ticket '${ticket.name}'`);
      return;
    }
    ...
    ticket.videos = ticket.videos.map(
      v => v.id == video.id ? video : v
    );

    actions.updateTicket(ticket).then(() => {
      // Url name needs to change to reflect new name
      if (this.titleChanged()) this.props.history.push(
  ...
  notesChanged = () => {
    return (this.state.newNotes.length > 0) ||
      (this.state.removedNotes.length > 0);
  }

  titleChanged = () => { return this.props.tickets.video.title != this.state.video.title; }

  detailsChanged = () => { return this.notesChanged() || this.titleChanged(); }
  ...
  displayTime = totalSeconds => {
    ...
    let secondsRounded = Number('0' + (totalSeconds % 60)).toFixed(2).slice(-5);
  ...
  deleteVideo = () => {
    ...
    const updatedVideoList = ticket.videos.filter(v => v.id !== tickets.video.id);
      ...
      <Container>
        ...
        {this.state.video.hasOwnProperty('id') &&
          <div className="mid-container">
            <EditVideoForm video={tickets.video}
              handleSubmit={this.handleSubmit}
              updateVideoState={this.updateVideoState}
              ticketStatus={tickets.selected.status}
              titleError={this.state.titleError} />
            <ReactPlayer url={tickets.video.fileLocation} ref={this.player}
              controls width="640" height="400" className="auto-center" />
            <div className="section">
              <NoteList notes={this.state.notes}
                displayTime={this.displayTime}
                btnOnClick={this.removeNote}
                seekTo={this.seekTo} />
              <NoteList notes={this.state.newNotes}
                displayTime={this.displayTime}
                btnOnClick={this.removeNewNote}
                seekTo={this.seekTo}
                header="New" />
              <NoteList notes={this.state.removedNotes}
                displayTime={this.displayTime}
                btnOnClick={this.reAddNote}
                seekTo={this.seekTo}
                icon="plus"
                header="To Remove" />
              <NoteInput inputRef={this.noteInput}
            ...
            {this.notesChanged() &&
              <div className="float-left">
                <Button color="grey" onClick={this.clearNoteChanges} className="float-left"
                  disabled={tickets.inProgress}>Clear Note Changes</Button>
              </div>
            }
            <div className="align-right auto-center">
              ...
              {tickets.selected.status == status.CLOSED ?
                <Button negative disabled>Delete</Button>
...
const mapDispatchToProps = (dispatch) => ({
  actions: bindActionCreators({ getTicketByName, updateTicket, selectVideo }, dispatch)
});

export default withRouter(connect(mapStateToProps, mapDispatchToProps)(VideoPageContainer));
