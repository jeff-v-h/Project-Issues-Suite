...
import { getProjectsForDropdown, getTicketsForDropdown } from '../../../helpers/utils';
...
class ProjectDropdownContainer extends React.PureComponent {
  ...
  favouriteProject = () => {
    ...
    this.props.actions.updateUser(user.signinName, user)
      .then(() => toastr.success(`'${project.name}' project saved to your favourites.`))
  ...
  isProjectFavourited = () => {
    ...
    if (users.data.favProjects.find(p => p.id == projects.selected.id)) return true;
    return false;
  }
        ...
        <MidDivWrapper >
          <div className="browse-fav-btn">
            {this.props.showTickets &&
              <FavButton isFavourited={this.isProjectFavourited()}
...
export default connect(mapStateToProps, mapDispatchToProps)(ProjectDropdownContainer);
