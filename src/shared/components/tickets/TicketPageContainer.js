...
class TicketPageContainer extends React.Component {
  constructor(props) {
    ...
    if (!ticket.hasOwnProperty('id')) {
      ...
      props.actions.getTicketByName(projectName, ticketName)
  ...
  handleSubmit = e => {
    ...
    const project = this.props.projects.data.find(p => p.name == ticket.projectName);
    ...
    if (nameChanged && nameExists) {
      this.setState({ nameError: true });
      toastr.error(`Name '${ticket.name}' already exists for project '${project.name}'`);
      return;
    }
    ...
    actions.updateTicket(ticket).then(() => {
      ...
      if (tickets.selected.name !== ticket.name) {
        ...
        this.props.history.push('/' +  encodeUrlComponent(project.name) + '/' +
  ...
  updateUserFavTicket = () => {
    ...
    user.favTickets = user.favTickets.map(
      ticketBase => ticketBase.id == newTicketBase.id ? newTicketBase : ticketBase
    );
  ...
  redirectToVideo = (video) => {
    return () => {
      ...
      this.props.history.push(getPath() + '/' + encodeUrlComponent(video.title));
    };
  }
  ...
  deleteTicket = () => {
    ...
    actions.deleteTicket(tickets.selected.id).then(() => {
  ...
  render() {
    ...
    const msg = "Are you sure you want to delete this ticket and ALL it's videos?";
          ...
          <div className="mid-container">
            ...
            <div className="big-auto-center">
              <div className="float-left">
                <FavButton isFavourited={this.isFavourite()}
            ...
            <VideoSection videos={state.ticket.videos}
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(TicketPageContainer));
